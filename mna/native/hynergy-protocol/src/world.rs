...
mod tests {
    ...
    fn world_command_buffer(commands: &[Vec<u8>]) -> Vec<u8> {
    ...
    #[test]
    fn empty_world_command_buffer_succeeds() {
        let mut engine = Engine::new();
        let world = engine.new_world().unwrap();

        apply_world_command_buffer(&mut engine, world, &world_command_buffer(&[])).unwrap();
    }

    #[test]
    fn add_wire_command_is_applied() {
        let mut engine = Engine::new();
        let world = engine.new_world().unwrap();

        let command = world_command(WORLD_COMMAND_ADD_WIRE, &1_u32.to_le_bytes());

        assert!(
            apply_world_command_buffer(&mut engine, world, &world_command_buffer(&[command]),)
                .is_ok()
        );
    ...
    #[test]
    fn commands_are_applied_eagerly_before_later_parse_failure() {
        let mut engine = Engine::new();
        let world = engine.new_world().unwrap();
        ...
        let error = apply_world_command_buffer(
            &mut engine,
            world,
            &world_command_buffer(&[first, malformed]),
        )
        .unwrap_err();
    ...
    #[test]
    fn commands_are_applied_eagerly_before_later_model_failure() {
        let mut engine = Engine::new();
        let world = engine.new_world().unwrap();

        let first = world_command(WORLD_COMMAND_ADD_WIRE, &1_u32.to_le_bytes());

        let duplicate = world_command(WORLD_COMMAND_ADD_WIRE, &1_u32.to_le_bytes());

        let error = apply_world_command_buffer(
            &mut engine,
            world,
            &world_command_buffer(&[first, duplicate]),
        )
        .unwrap_err();
    ...
    #[test]
    fn zero_non_zero_id_is_rejected_by_decoder() {
        let mut engine = Engine::new();
        let world = engine.new_world().unwrap();

        let command = world_command(WORLD_COMMAND_ADD_WIRE, &0_u32.to_le_bytes());

        let error =
            apply_world_command_buffer(&mut engine, world, &world_command_buffer(&[command]))
                .unwrap_err();

        assert_eq!(error.kind(), WorldCommandErrorKind::InvalidId);
        assert_eq!(error.command_index(), 0);
    }

    #[test]
    fn unknown_world_is_a_pre_command_error() {
        let mut engine = Engine::new();

        let error =
            apply_world_command_buffer(&mut engine, 123, &world_command_buffer(&[])).unwrap_err();

        assert_eq!(error.kind(), WorldCommandErrorKind::UnknownWorld);
        assert_eq!(error.command_index(), u32::MAX);
        assert_eq!(error.byte_offset(), u32::MAX);
    }
}
